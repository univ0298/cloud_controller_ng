...
class DropletsController < ApplicationController
  ...
  def send_droplet_blob(droplet)
    if droplet.blobstore_key is nil
      raise CloudController::Errors::ApiError.new_from_details('blobstore_key not present on droplet. This may be due to a failed build.')
    end
