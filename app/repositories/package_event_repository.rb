  ...
  module Repositories
    class PackageEventRepository
      def self.record_app_package_create(package, user_audit_info, request_attrs)
        ...
        VCAP::AppEventEmitter.emit(app_guid, "Adding app package for app with guid #{app_guid}")
      ...
      def self.record_app_package_copy(package, user_audit_info, source_package_guid)
        app = package.app
        VCAP::AppEventEmitter.emit(app.guid, "Adding app package for app with guid #{app.guid} copied from package with guid #{source_package_guid}")
        metadata = {
          package_guid: package.guid,
          request:      {
      ...
      def self.record_app_package_upload(package, user_audit_info)
        VCAP::AppEventEmitter.emit(package.app.guid, "Uploading app package for app with guid #{package.app.guid}")
      ...
      def self.record_app_upload_bits(package, user_audit_info)
        VCAP::AppEventEmitter.emit(package.app.guid, "Uploading bits for app with guid #{package.app.guid}")
        metadata = { package_guid: package.guid }
      ...
      def self.record_app_package_delete(package, user_audit_info)
        VCAP::AppEventEmitter.emit(package.app.guid, "Deleting app package for app with guid #{package.app.guid}")
        metadata = { package_guid: package.guid }
      ...
      def self.record_app_package_download(package, user_audit_info)
        VCAP::AppEventEmitter.emit(package.app.guid, "Downloading app package for app with guid #{package.app.guid}")
        metadata = { package_guid: package.guid }
