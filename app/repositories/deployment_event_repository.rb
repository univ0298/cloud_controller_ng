  ...
  module Repositories
    class DeploymentEventRepository
      def self.record_create(deployment, droplet, user_audit_info, v3_app_name, space_guid, org_guid, params, type)
        VCAP::AppEventEmitter.emit(deployment.app_guid, "Creating deployment for app with guid #{deployment.app_guid}")
      ...
      def self.record_cancel(deployment, droplet, user_audit_info, v3_app_name, space_guid, org_guid)
        VCAP::AppEventEmitter.emit(deployment.app_guid, "Cancelling deployment for app with guid #{deployment.app_guid}")
