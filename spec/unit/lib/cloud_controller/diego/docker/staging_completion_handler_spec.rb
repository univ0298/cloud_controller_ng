    ...
    module Docker
      RSpec.describe StagingCompletionHandler do
        ...
        before do
          ...
          allow(VCAP::AppEventEmitter).to receive(:emit_error)
            ...
            context 'when staging result is returned' do
              ...
              context 'when process_types is empty' do
                ...
                it 'logs an error for the CF user' do
                  ...
                  expect(VCAP::AppEventEmitter).to have_received(:emit_error).with(build.app_guid, /No process types returned from stager/)
                end
              end
          ...
          context 'failure' do
            ...
            context 'when the staging fails' do
              ...
              it 'should emit a loggregator error' do
                expect(VCAP::AppEventEmitter).to receive(:emit_error).with(build.app_guid, /Insufficient resources/)
                handler.staging_complete(payload)
              end
            end
            ...
            context 'with a malformed success message' do
              ...
              it 'logs an error for the CF user' do
                expect(VCAP::AppEventEmitter).to have_received(:emit_error).with(build.app_guid, /Malformed message from Diego stager/)
              end
