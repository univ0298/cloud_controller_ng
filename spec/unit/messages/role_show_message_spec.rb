...
module VCAP::CloudController
  RSpec.describe RoleShowMessage do
    ...
    it 'does not accept include that is not user or organizaition or space' do
      message = RoleShowMessage.from_params({ 'include' => 'user' })
      expect(message).to be_valid
      message = RoleListMessage.from_params({ 'include' => 'organization' })
      expect(message).to be_valid
      message = RoleListMessage.from_params({ 'include' => 'space' })
      expect(message).to be_valid
