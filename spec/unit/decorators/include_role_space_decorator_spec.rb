...
module VCAP::CloudController
  RSpec.describe IncludeRoleSpaceDecorator do
    ...
    let(:roles) { [Role.make(space: space1), Role.make(space: space1)] }
    ...
    it 'decorates the given hash with spaces from roles' do
      ...
      expect(hash[:included][:space]).to match_array([Presenters::V3::SpacePresenter.new(space1).to_hash,
    ...
    it 'does not overwrite other included fields' do
      ...
      hash = subject.decorate(wreathless_hash, roles)
      expect(hash[:foo]).to eq('bar')
      expect(hash[:included][:spaces]).to match_array([Presenters::V3::SpacePresenter.new(space1).to_hash,
                                                              Presenters::V3::SpacePresenter.new(space2).to_hash])
